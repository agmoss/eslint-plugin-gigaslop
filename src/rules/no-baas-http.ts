...
function findBlockedHost(value: string, hosts: readonly string[]): string | null {
...
export const noBaasHttp = createRule<RuleOptions, MessageIds>({
    ...
    schema: [
      {
        type: 'object',
        properties: {
          additionalHosts: { type: 'array', items: { type: 'string' }, uniqueItems: true },
          allowHosts: { type: 'array', items: { type: 'string' }, uniqueItems: true },
        },
  ...
  create(context, [options]) {
    const allow = new Set((options.allowHosts ?? []).map((host) => host.toLowerCase()));
    const hosts = DEFAULT_BAAS_HTTP_HOSTS.concat(options.additionalHosts ?? []).filter(
      (host) => !allow.has(host.toLowerCase()),
    );

    function check(node: TSESTree.Node, value: string): void {
      const host = findBlockedHost(value, hosts);
      if (host) context.report({ node, messageId: 'blockedHost', data: { host } });
    }
